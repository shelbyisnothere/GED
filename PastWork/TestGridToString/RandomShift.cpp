...
RandomShift::RandomShift(Curve curve, int g)
: g(g), n(curve.size())
{
    //translate points to Point_3
    for(Point_2 p : curve){
        Point_3 newP(p.x(), p.y(), 0);
        pts.push_back(newP);
    }
}

void RandomShift::shiftGrid() {
...
vector<Cell> RandomShift::getString() { return string; }
